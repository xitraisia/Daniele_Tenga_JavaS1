...
function Encounters() {
    ...
    useEffect(() => {
        fetch("http://localhost:8080/api/encounter")
            ...
            .catch(console.log);
    }, [refresh]);// intead of refreshign encounters, get all encounters and rebuild the whole page
    ...
    function addClick() {
        ...
        const time = new Intl.DateTimeFormat("en-US", { hour: "2-digit", minute: "2-digit" })
            .format(now).replace(" ", "");
        console.log(time);
        setScopedEncounter({ id: 0, brief: "", details: "", investigators: [], dateTime: `${day}-${month}-${year} ${time}` });
        setShowForm(true);
    }
