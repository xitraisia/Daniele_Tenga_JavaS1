import Form from "./components3/Form.js";
import Table from "./components3/Table.js";
import api from "./api/tshirt.js";
const state = {
  data: [],
};
...
function render() {
  ...
  root.querySelector("form")?.addEventListener("submit", (event) => {
    ...
    api
      .create(newTshirt)
      .then(() => {
        state.data = [...state.data, newTshirt];
        render();
      })
      .catch((err) => {
        console.log(err);
  ...
  root.querySelectorAll(".btn-warning").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      api.update({[key4Update]: value4Update, id: id4Update }).then(() => {
        const tshirt2Update = state.data.find(
